document.addEventListener("DOMContentLoaded", () => {
  ...
  const idadeInput = document.getElementById("idadePaciente"); // AGORA É O CRF
  ...
  const ultimaConsultaInput = document.getElementById("ultimaConsultaPaciente"); // AGORA É DATA DE CONTRATAÇÃO
  const statusInput = document.getElementById("statusPaciente"); // AGORA É SITUAÇÃO: ATIVO/INATIVO

  // ===== DADOS DOS FARMACÊUTICOS =====
  let todosPacientes = [
    {
      id: 1,
      nome: "Dra. Ana Lúcia Mendes",
      idade: "SP-87654", // CRF
      sexo: "Feminino",
      cpf: "111.111.111-11",
      telefone: "+(11) 99999-9999",
      endereco: "Rua das Flores, 123, São Paulo",
      ultimaConsulta: "15/03/2023", // Data de contratação
      status: "Ativo",
      prontuario: [
        {
          data: "20/05/2024",
          ...
          farmaceutico: "Dra. Ana Lúcia Mendes - CRF-SP 87654"
        },
        {
          data: "10/05/2024",
    ...
    },
    {
      ...
      ultimaConsulta: "22/07/2023", // Data de contratação
      status: "Inativo",
      prontuario: []
    }
  ];
  ...
  function criarCard(paciente) {
    ...
    card.innerHTML = `
      <div class="paciente-info">
        ...
        <div>
          ...
          <p class="text-muted">CRF: ${paciente.idade} • ${paciente.sexo} • ${paciente.telefone}</p>
          <small class="text-secondary">CPF: ${paciente.cpf} • Contratado em: ${paciente.ultimaConsulta}</small>
        </div>
      </div>
      <div class="paciente-actions">
        <span class="status-badge ${statusNorm}">${paciente.status || 'Sem situação'}</span>
        <button class="btn btn-outline-secondary btn-sm btn-prontuario"><i class="fa fa-file-medical"></i> Atendimentos</button>
      </div>
    `;
    ...
    // Botão "Atendimentos" → abre modal de prontuário
  ...
  function abrirDetalhes(id) {
    ...
    detalhesCorpo.innerHTML = `
      <div class="row g-3">
        <div class="col-12">
          <h5><i class="fa fa-user-md"></i> ${paciente.nome}</h5>
        </div>
        <div class="col-6">
          <label class="form-label"><strong>CRF</strong></label>
          <p class="form-control-plaintext">${paciente.idade}</p>
        </div>
        <div class="col-6">
        ...
        <div class="col-6">
          <label class="form-label"><strong>Data de Contratação</strong></label>
  ...
  btnEditarDoDetalhe.addEventListener("click", () => {
    ...
    idadeInput.value = paciente.idade; // CRF
    ...
    modalTitle.textContent = "Editar Farmacêutico";

    bootstrap.Modal.getInstance(detalhesModal)?.hide();
    new bootstrap.Modal(pacienteModal).show();
  });

  // ===== ABRIR PRONTUÁRIO (ATENDIMENTOS REALIZADOS PELO FARMACÊUTICO) =====
    ...
    if (prontuario.length === 0) {
      prontuarioCorpo.innerHTML = `
        <div class="text-center py-4">
          ...
          <p class="text-muted mt-3">Nenhum atendimento registrado por este farmacêutico.</p>
        </div>
      `;
    } else {
      ...
      prontuarioCorpo.innerHTML = `
        <h5><i class="fa fa-file-medical"></i> Atendimentos Realizados</h5>
        <p><strong>Farmacêutico:</strong> ${paciente.nome}</p>
        <hr>
        ${atendimentosHTML}
      `;
    }
  ...
  function aplicarFiltros() {
    ...
    const filtrados = todosPacientes.filter(p => {
      const matchBusca =
        ...
        p.idade.includes(termo); // Busca também por CRF

      // ✅ CORREÇÃO AQUI: FILTRO EXATO POR STATUS
      let matchFiltro = true;

      if (filtro === "ativo") {
        matchFiltro = p.status === "Ativo";
      } else if (filtro === "inativo") {
        matchFiltro = p.status === "Inativo";
      }
  ...
  formPaciente.addEventListener("submit", function (e) {
    ...
    formPaciente.reset();
    pacienteId.value = "";
    modalTitle.textContent = "Cadastrar Novo Farmacêutico";
