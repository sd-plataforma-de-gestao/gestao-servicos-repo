...
// Função para limpar CPF (remove tudo que não é dígito)
...
function validarCPF($cpf)
{
    ...
    if (preg_match('/^(\d)\1+$/', $cpf)) return false; // Todos os dígitos iguais
...
if (isset($_GET['action']) && $_GET['action'] === 'load_list') {
    ...
    if ($result && mysqli_num_rows($result) > 0):
        ...
        while ($row = mysqli_fetch_assoc($result)):
            ...
            echo '<td>' . (!empty($row['cpf']) ? htmlspecialchars(formatarCPF($row['cpf'])) : '-') . '</td>';
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            ...
                            <div class="col-6">
                                ...
                                <select class="form-select" name="tipo_paciente" required>
