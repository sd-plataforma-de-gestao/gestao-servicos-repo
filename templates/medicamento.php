<?php
// Certifique-se de que o session_start() está ativo se você usar $_SESSION['error'] ou $_SESSION['success']
// session_start(); 
...
// Usa require_once para evitar múltiplas inclusões
require_once("../config/database.php");

// =========================================================================
// Bloco AJAX: Carregar Lista de Medicamentos
// =========================================================================
if (isset($_GET['action']) && $_GET['action'] === 'load_list') {
    ...
    if (!$stmt) {
        // Retorna um erro formatado para o AJAX
        header('Content-Type: text/plain');
        echo '<p class="text-danger">Erro ao preparar a consulta: ' . $conn->error . '</p>';
        exit;
    }

    // Vincula os parâmetros se houverem
    if (!empty($params)) {
        $types = str_repeat('s', count($params));
        // O operador spread (...) desempacota o array de parâmetros
    ...
    if ($result && $result->num_rows > 0):
        // Usa $result->fetch_assoc() que é padrão para mysqli_result
        ...
        while ($row = $result->fetch_assoc()):
            ...
            // Lógica de cores para os badges de estoque
            ...
            // Badge com informação de estoque
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Flag para verificar se a requisição veio via AJAX
    ...
    // CORREÇÃO DE SEGURANÇA E TIPO DE DADO: Limpeza do campo preço para double
    ...
    if (empty($nome) || empty($principio_ativo) || empty($dosagem) || empty($laboratorio) || empty($tipo) || empty($numero_lote) || empty($data_validade)) {
        if ($isAjax) {
            ...
            // Se não for AJAX, você pode usar a sessão
    ...
    // Vincula os parâmetros. 'd' é para double/float (Preço)
    $stmt->bind_param(
        "sssssssiidss",
        $nome,
        $principio_ativo,
    ...
    if ($stmt->execute()) {
        if ($isAjax) {
            header('Content-Type: text/plain');
            echo "success"; // Retorno de sucesso para o AJAX
        } else {
        ...
        if ($isAjax) {
            header('Content-Type: text/plain');
            echo "error: " . $stmt->error; // Retorno de erro para o AJAX
        } else {
...
<head>
    ...
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script> <!-- Adicionado o JS do SweetAlert2 -->
...
<body>
            ...
            <div class="medicamentos-page">
                ...
                <div class="medicamentos-list card">
                    ...
                    <div id="lista-pacientes">
                        <?php
                        // CÓDIGO PHP DE EXIBIÇÃO INICIAL DA TABELA
    ...
    <script>
        // Função para carregar templates (header/sidebar)
        ...
        // Função para carregar a lista de medicamentos com filtros (AJAX)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Carrega a lista inicial (mesmo após o DOMContentLoaded, para garantir)
            carregarListaComFiltros();
            
            // LÓGICA AJAX PARA CADASTRO DE MEDICAMENTO (MODAL)
            if (formMedicamento) {
                formMedicamento.addEventListener("submit", function(e) {
                    ...
                    fetch('medicamento.php', {
                        ...
                        body: formData,
                        headers: {
                            "X-Requested-With": "XMLHttpRequest" // Identifica a requisição como AJAX para o PHP
                        }
                    })
                    .then(response => response.text())
                    .then(result => {
                        ...
                        if (message === "success") {
                            ...
                            carregarListaComFiltros(); // Atualiza a tabela
                            
                            // CHAMA O POP-UP ESTILIZADO (Swal.fire)
                            Swal.fire({
                                icon: 'success',
                                title: 'Sucesso!',
                                text: 'Medicamento cadastrado com sucesso!',
                                confirmButtonColor: '#1C5B40' // Sua Cor Principal (opcional)
                            });
                            ...
                            Swal.fire({
                                ...
                                text: errorMessage,
                                confirmButtonColor: '#DC3545' // Cor de erro (opcional)
                            });
                        }
                    })
                    .catch(error => {
